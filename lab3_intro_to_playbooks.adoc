...
Now that we know the structure of a playbook and how easy could be to write one, let's discuss the ansible-playbook command. The ansible-playbook command is used to run playbooks and is executed in the control (in this case your `workstation` ) node. Using this command could be as simple as the following example: 
...
Because we are using RHEL 7.5 in our lab and there is a dependency issue on `pip`, we are going to manually create a virtual environment and install `shade`, `setuptools`, and `python-openstackclient`. 
...
$ (shade) pip install python-openstackclient
----


Verification that `shade` and `python-openstackclient` and was installed can be done via an ad-hoc command as
follows:
